...
class CL_FNGroupPosItem(QtWidgets.QDialog):
    def __init__(self):
        super(CL_FNGroupPosItem, self).__init__()
        cwd = Path.cwd()
    ...
    def FN_LOAD_CREATE(self):
        filename = self.dirname + '/Pos_Group_Pos_item.ui'
    ...
    def FN_GetGroup(self):
        try:
            self.conn = db1.connect()
            mycursor = self.conn.cursor()
            mycursor.execute("SELECT GROUP_NAME , GROUP_ID FROM POS_QUICK_GROUP")
            records = mycursor.fetchall()
            print(records)
            for row, val in records:
