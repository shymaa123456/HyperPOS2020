...
class CL_FNGroup(QtWidgets.QDialog):
    def __init__(self):
        super(CL_FNGroup, self).__init__()
        cwd = Path.cwd()
    ...
    def FN_LOAD_CREATE(self):
        filename = self.dirname + '/Pos_Group.ui'
    ...
    def FN_Create(self):
        ...
        sql_select_Query = "select * from POS_FN_GROUP where GROUP_NAME = %s "
        x = (indx,)
        ...
        if mycursor.rowcount > 0:
            ...
            if len(self.group_name.text()) == 0:
                ...
                try:
                    sql0 = "  LOCK TABLES Hyper1_Retail.POS_FN_GROUP WRITE  "
                    mycursor.execute(sql0)
                    sql = "INSERT INTO POS_FN_GROUP (GROUP_NAME,NOTES,STATUS)" \
                          " VALUES (%s, %s, %s) "
                    val = (
                        self.group_name.text().strip(), self.notes.toPlainText().strip(),
                        self.CMB_Status.currentIndex())
                    mycursor.execute(sql, val)
                    mycursor.execute("SELECT * FROM POS_FN_GROUP Where GROUP_NAME = '" + indx + "'")
                    c = mycursor.fetchone()
                    id = c[0]
                    sql00 = "  UNLOCK   tables    "
