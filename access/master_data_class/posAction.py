...
class CL_posAction(QtWidgets.QDialog):
    dirname = ''
    switch_window = QtCore.pyqtSignal()
    def __init__(self):
        super(CL_posAction, self).__init__()
        cwd = Path.cwd()
    ...
    def FN_LOAD_DISPlAY(self):
        filename = self.dirname + '/posAction.ui'
        ...
        try:
            ...
            self.CMB_reqApproval.addItem("Yes", '1')
            self.CMB_reqApproval.addItem("No", '0')
    ...
    def FN_SEARCH(self):
        ...
        try:
            ...
            if name != '' :
                whereClause = whereClause + "and `ACTION_DESC` like '%" + str(name) + "%'"

            sql_select_query = "SELECT  `ACTION_ID`,`ACTION_DESC`, `STATUS`,APPROVAL_REQUIRED ,NOTES FROM `Hyper1_Retail`.`POS_ACTION` " + whereClause + "  order by ACTION_ID asc"
    ...
    def FN_GET_ALL(self):
        ...
        try:
            ...
            mycursor.execute("SELECT  `ACTION_ID`,`ACTION_DESC`, `STATUS` ,APPROVAL_REQUIRED ,NOTES FROM `Hyper1_Retail`.`POS_ACTION` order by ACTION_ID   asc")
            records = mycursor.fetchall()
            for row_number, row_data in enumerate(records):
                self.Qtable.insertRow(row_number)
                for column_number, data in enumerate(row_data):
    ...
    def FN_GET_ONE(self):
        try:
            if len(self.Qtable.selectedIndexes()) >= 0:
                ...
                reqApproval = self.Qtable.item(rowNo, 3).text()
                ...
                self.CMB_reqApproval.setCurrentText(reqApproval)
    ...
    def FN_CHECK_DUP_NAME(self,name,id=''):
        ...
        sql = "SELECT ACTION_DESC  FROM Hyper1_Retail.POS_ACTION where ACTION_DESC = '"+name+"' and ACTION_ID !='"+id+"'"
    ...
    def FN_CREATE(self):
        ...
        status = self.CMB_status.currentData()
        reqApproval = self.CMB_reqApproval.currentData()
        ...
        if self.name == '' :
            ...
            try:
                if self.FN_CHECK_DUP_NAME(self.name) != False:
                    ...
                    sql = "INSERT INTO Hyper1_Retail.POS_ACTION( ACTION_DESC , STATUS,APPROVAL_REQUIRED,notes) " \
                          "         VALUES (  %s, %s,%s,%s)"

                    val = (self.name,  status , reqApproval,notes  )
        ...
        if len(self.Qtable.selectedIndexes()) >0 :
            ...
            if self.desc == '':
                ...
                if error!=1:
                    ...
                    sql = "UPDATE `Hyper1_Retail`.POS_ACTION SET `ACTION_DESC` = %s, STATUS = %s ,APPROVAL_REQUIRED = %s , NOTES= %s" \
                          " WHERE `ACTION_ID` = %s"
                    val = (desc,status,reqApproval,notes, id)
                    mycursor.execute(sql, val)
                    #mycursor.close()
                    #
                    ...
                    if str(status) != str(status_old):
                        util.FN_INSERT_IN_LOG("POS_ACTION", "status", status, status_old, id)
        else:
