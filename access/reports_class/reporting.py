    ...
    def FN_loadData(self, PROM_ID):
        ...
        if self.cond == 1:
            ...
            self.query = (
                    ...
                    'yyyy-MM-dd') + " " + self.Qtime_from.dateTime().toString(
                    'hh:mm:ss') + "' and `PROMOTION_DETAIL`.`PROM_END_DATE`<='" + self.Qdate_to.dateTime().toString(
                    'yyyy-MM-dd') + " " + self.Qtime_to.dateTime().toString(
                    'hh:mm:ss') + "' JOIN `PROM_BRANCH` ON `PROM_BRANCH`.`BRANCH_NO`=(SELECT `BRANCH`.`BRANCH_NO` FROM `BRANCH` WHERE `BRANCH`.`BRANCH_DESC_A`='" + self.Qcombo_branch.currentText() + "')")
        elif self.cond == 2:
            self.query = (
            ...
            self.query = (
                        "SELECT `PROMOTION_HEADER`.`PROM_ID`, `PROMOTION_HEADER`.`PROM_TYPE_ID`, `PROMOTION_HEADER`.`PROM_CREATED_BY`, `PROMOTION_HEADER`.`PROM_CREATED_ON`, `PROMOTION_DETAIL`.`PROM_LINE_NO`, `PROMOTION_DETAIL`.`POS_ITEM_NO`,`PROMOTION_DETAIL`.`POS_GTIN`,`PROMOTION_DETAIL`.`BMC_ID`,`PROMOTION_DETAIL`.`PROM_PRICE_BEFORE_DISC`,`PROMOTION_DETAIL`.`PROM_ITEM_SCALE_FLAG`,`PROMOTION_DETAIL`.`PROM_GROUP_SCALE_FLAG`,`PROMOTION_DETAIL`.`PROM_DISCOUNT_FLAG`,`PROMOTION_DETAIL`.`PROM_ITEM_QTY`,`PROMOTION_DETAIL`.`PROM_ITEM_DISC_VAL`,`PROMOTION_DETAIL`.`PROM_ITEM_PRICE`,`PROMOTION_DETAIL`.`PROM_START_DATE`,`PROMOTION_DETAIL`.`PROM_END_DATE`,`PROMOTION_DETAIL`.`PROM_STATUS` FROM `PROMOTION_HEADER` JOIN `PROMOTION_DETAIL` ON `PROMOTION_HEADER`.`PROM_ID`=`PROMOTION_DETAIL`.`PROM_ID` AND `PROMOTION_DETAIL`.`PROM_STATUS`='" + str(
                    self.status) + "' JOIN `PROM_BRANCH` ON `PROM_BRANCH`.`BRANCH_NO`=(SELECT `BRANCH`.`BRANCH_NO` FROM `BRANCH` WHERE `BRANCH`.`BRANCH_DESC_A`='" + self.Qcombo_branch.currentText() + "')")
        elif self.cond == 11:
            self.query = (
                        "SELECT `PROMOTION_HEADER`.`PROM_ID`, `PROMOTION_HEADER`.`PROM_TYPE_ID`, `PROMOTION_HEADER`.`PROM_CREATED_BY`, `PROMOTION_HEADER`.`PROM_CREATED_ON`, `PROMOTION_DETAIL`.`PROM_LINE_NO`, `PROMOTION_DETAIL`.`POS_ITEM_NO`,`PROMOTION_DETAIL`.`POS_GTIN`,`PROMOTION_DETAIL`.`BMC_ID`,`PROMOTION_DETAIL`.`PROM_PRICE_BEFORE_DISC`,`PROMOTION_DETAIL`.`PROM_ITEM_SCALE_FLAG`,`PROMOTION_DETAIL`.`PROM_GROUP_SCALE_FLAG`,`PROMOTION_DETAIL`.`PROM_DISCOUNT_FLAG`,`PROMOTION_DETAIL`.`PROM_ITEM_QTY`,`PROMOTION_DETAIL`.`PROM_ITEM_DISC_VAL`,`PROMOTION_DETAIL`.`PROM_ITEM_PRICE`,`PROMOTION_DETAIL`.`PROM_START_DATE`,`PROMOTION_DETAIL`.`PROM_END_DATE`,`PROMOTION_DETAIL`.`PROM_STATUS` FROM `PROMOTION_HEADER` JOIN `PROMOTION_DETAIL` ON `PROMOTION_HEADER`.`PROM_ID`=`PROMOTION_DETAIL`.`PROM_ID` JOIN `PROM_BRANCH` ON `PROM_BRANCH`.`BRANCH_NO`=(SELECT `BRANCH`.`BRANCH_NO` FROM `BRANCH` WHERE `BRANCH`.`BRANCH_DESC_A`='" + self.Qcombo_branch.currentText() + "')")
